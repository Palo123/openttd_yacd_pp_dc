		...
		for (Vehicle *v = front_v; v != NULL; v = v->Next()) {
			if (v->cargo_cap > 0 && v->cargo.TotalCount() > 0) {
				v->cargo.Stage(
						...
						front_v->last_station_visited, front_v->last_order_id, front_v->current_order.GetUnloadType(), v->cargo_type, front_v->cargo_payment);
 ...
 */
static void ReserveConsist(Station *st, Vehicle *u, CargoArray *consist_capleft, OrderID cur_order)
{
	...
	while (next_cargo != NULL) {
		...
		for (; v != NULL; v = v->Next()) {
			...
			if (cap > 0) {
				cap -= st->goods[v->cargo_type].cargo.Reserve(cap, &v->cargo, st->xy, cur_order);
			}
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (front->current_order.IsRefit() && artic_part == 1 && IsArticulatedVehicleEmpty(v) &&
			...
			if (new_cid == CT_AUTO_REFIT) {
				...
				FOR_EACH_SET_CARGO_ID(cid, refit_mask) {
					...
					if (st->goods[cid].cargo.HasCargoFor(last_order) || st->goods[cid].cargo.HasCargoFor(INVALID_ORDER)) {
			...
			w = v_start;
			do {
				st->goods[w->cargo_type].cargo.Reserve(w->cargo_cap, &w->cargo, st->xy, last_order);
		...
		if (cap_left > 0 && (v->cargo.ActionCount(VehicleCargoList::MTA_LOAD) > 0 || ge->cargo.AvailableCount() > 0)) {
			...
			uint loaded = ge->cargo.Load(cap_left, &v->cargo, st->xy, last_order);
