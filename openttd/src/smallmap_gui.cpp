...
#define MCS(height)  {0, STR_TINY_BLACK_HEIGHT, INVALID_INDUSTRYTYPE, height, INVALID_COMPANY, INVALID_CARGO, true, false, true}
 ...
 */
void SmallMapWindow::DrawRouteLinks() const
{
...
{
	uint num_rows = CeilDiv(max(_smallmap_company_count, max(_smallmap_industry_count, _smallmap_cargo_count)), columns);
			...
			if (this->map_type == SMT_INDUSTRY || this->map_type == SMT_OWNER || this->map_type == SMT_ROUTE_LINKS) {
				...
				if (this->map_type == SMT_INDUSTRY) {
					...
					if (click_pos < _smallmap_cargo_count) {
						this->SelectLegendItem(click_pos, _legend_from_cargoes, _smallmap_cargo_count);
					}
				}
				this->SetDirty();
			}
